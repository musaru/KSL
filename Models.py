...
################################################################################################  
################################################################################################  
class ReLU_Channel_Attention(nn.Module):
...
class Sigmoid_Channel_Attention(nn.Module):
    def __init__(self,out_channels):
        super().__init__()
...
################################################################################################  
################################################################################################  
class KSL_GDN(nn.Module):
    def __init__(self,out_channels,kernel_size,stride,padding):
        super().__init__()
        
        self.ksl_gdn=nn.Sequential(
            nn.Conv2d(out_channels,
               out_channels,
               (kernel_size[0], 1),
               (stride, 1),
    ...
    def forward(self,x):
        return self.ksl_gdn(x)
################################################################################################  
################################################################################################  
################################################################################################  
class CA_KG(nn.Module):
    def __init__(self):
        super().__init__()
            
        self.ca_kg = nn.Sequential(
            
            ReLU_Channel_Attention(out_channels=256),
            # Sigmoid_Channel_Attention(out_channels=256),
            
            KSL_GDN(out_channels=256,kernel_size=(3,1),stride=1,padding=0),
            # KSL_GDN(out_channels=256,kernel_size=(5,1),stride=1,padding=1),
        )
...
class modified_st_gcn(nn.Module):
    """Applies a spatial temporal graph convolution over an input graph sequence.
    Args:
        - in_channels: (int) Number of channels in the input sequence data.
        - out_channels: (int) Number of channels produced by the convolution.
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.channel_attention_module = Sigmoid_Channel_Attention(out_channels)
    def forward(self, x, A):
        res = self.residual(x)
        # print(res)
        x = self.gcn(x, A)
...
class ModifiedStreamSpatialTemporalGraph(nn.Module):
    """Spatial temporal graph convolutional networks.
    Args:
        - in_channels: (int) Number of input channels.
        - graph_args: (dict) Args map of `Actionsrecognition.Utils.Graph` Class.
    ...
    def __init__(self, in_channels, graph_args, num_class=None,
        ...
        self.data_bn = nn.BatchNorm1d(in_channels * A.size(1))
        self.st_gcn_networks = nn.ModuleList((
            modified_st_gcn(in_channels, 64, kernel_size, 1, residual=False, **kwargs0),
            modified_st_gcn(64, 64, kernel_size, 1, **kwargs),
            modified_st_gcn(64, 64, kernel_size, 1, **kwargs),
            modified_st_gcn(64, 64, kernel_size, 1, **kwargs),
            modified_st_gcn(64, 128, kernel_size, 2, **kwargs),
            modified_st_gcn(128, 128, kernel_size, 1, **kwargs),
            modified_st_gcn(128, 128, kernel_size, 1, **kwargs),
            modified_st_gcn(128, 256, kernel_size, 2, **kwargs),
            modified_st_gcn(256, 256, kernel_size, 1, **kwargs),
            modified_st_gcn(256, 256, kernel_size, 1, **kwargs)
        ))
...
class ProposedTwoStreamSpatialTemporalGraph(nn.Module):
    """Two inputs spatial temporal graph convolutional networks.
    Args:
        - graph_args: (dict) Args map of `Actionsrecognition.Utils.Graph` Class.
        - num_class: (int) Number of class outputs.
    ...
    def __init__(self, graph_args, num_class, edge_importance_weighting=True,
                 **kwargs):
        super().__init__()
        self.pts_stream = ModifiedStreamSpatialTemporalGraph(3, graph_args, None,
                                                     edge_importance_weighting,
                                                     **kwargs)
        self.mot_stream = ModifiedStreamSpatialTemporalGraph(3, graph_args, None,
                                                     edge_importance_weighting,
                                                     **kwargs)
    ...
    def forward(self, inputs):
        ...
        out1 = self.ca_kg1(out1)
        out2 = self.ca_kg2(out2)

        concat = torch.cat([out1, out2], dim=-1)
        # concat = torch.cat([out2], dim=-1)
        
        out = self.fcn(concat)
        return out
    
    
class AblationExperimentModel1(nn.Module):
    """Two inputs spatial temporal graph convolutional networks.
    Args:
        - graph_args: (dict) Args map of `Actionsrecognition.Utils.Graph` Class.
        - num_class: (int) Number of class outputs.
...
class AblationExperimentModel2(nn.Module):
    """Two inputs spatial temporal graph convolutional networks.
    Args:
        - graph_args: (dict) Args map of `Actionsrecognition.Utils.Graph` Class.
        - num_class: (int) Number of class outputs.
...
class AblationExperimentModel3(nn.Module):
    """Two inputs spatial temporal graph convolutional networks.
    Args:
        - graph_args: (dict) Args map of `Actionsrecognition.Utils.Graph` Class.
        - num_class: (int) Number of class outputs.
    ...
    def __init__(self, graph_args, num_class, edge_importance_weighting=True,
                 **kwargs):
        super(AblationExperimentModel3,self).__init__()
        self.pts_stream = ModifiedStreamSpatialTemporalGraph(3, graph_args, None,
                                                     edge_importance_weighting,
                                                     **kwargs)
        self.mot_stream = ModifiedStreamSpatialTemporalGraph(3, graph_args, None,
